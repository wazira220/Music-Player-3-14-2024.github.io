...
void draw() {
  ...
  if ( playList[currentSong].isPlaying() ) {
    //Empty IF is FALSE
  } else if ( playList[currentSong].length() < 120000 ) { //PAIN Minutes is 2, 120s, 120,000ms
    //TRUE: if song is less than 2 minutes, STOP, I want to hear it from the beginning
...
void keyPressed() {
  ...
  if ( key=='I' || key=='l' ) { //Loop Infinite Times
